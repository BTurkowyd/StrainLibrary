...
class NewStrainForm(FlaskForm):
    ...
    selection_marker = SelectField('Selection marker', validators=[Optional()], coerce=int)
    box = SelectField('Box', validators=[DataRequired()], coerce=int)
...
class NewSelectionMarkerForm(FlaskForm):
    ...
    def validate_name(self, name):
        selection_marker = SelectionMarker.query.filter_by(name=name.data).first()
        if selection_marker:
